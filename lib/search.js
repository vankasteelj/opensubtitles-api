    ...
    optimizeSubs(response, input) {
        ...
        return Promise.all(response.map(sub => {
            ...
            const tmp = {
                ...
                utf8Url: input.gzip ? sub.SubDownloadLink.replace('download/', 'download/subencoding-utf8/') : sub.SubDownloadLink.replace('.gz', '').replace('download/', 'download/subencoding-utf8/')
            }
