...
module.exports = new class LibSearch {
    ...
    optimizeQueryTerms(input) {
        // parse
        let normalize = this.normalizeProt();
        ...
        input.tags = input.filename && normalize(input.filename).toLowerCase().match(/[a-z0-9]{2,}/gi)
    ...
    optimizeSubs(response, input) {
        ...
        const matchTags = (sub, maxScore) => {
            ...
            if (!input.tags || !input.tags.length) return 0
        ...
        return Promise.all(response.map(sub => {
            ...
            sub.hash = sub.MovieHash.toLowerCase()
            sub.fps = sub.MovieFPS && parseInt(sub.MovieFPS) > 0 && sub.MovieFPS

            // check: extencion, imdb, episode
            ...
            if (input.imdbid && sub.imdbid === input.imdbid)
                tmp.score += 5
            if (input.season && input.episode && sub.season === input.season && sub.episode === input.episode)
                tmp.score += 2
