...
LibSearch.prototype.optimizeSubs = function(response, input) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        _.each(response, function(sub) {
            ...
            if (!sub || input.extensions.indexOf(sub.SubFormat) == -1) {
                return;
            }
            ...
            // episode check
            if (input.season && input.episode) {
                if (sub.SeriesSeason !== parseInt(input.season).toString() || sub.SeriesEpisode !== parseInt(input.episode).toString()) {
                    return;
                }
            }
